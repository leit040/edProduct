...
@section('content')
    ...
    <h1>@if(!$type)Create @else Update @endif Type</h1>
    <form method="POST" @if(!$type)  action="{{route('types.store')}}"    enctype="multipart/form-data"> @else action="{{route('types.update',$type->id)}}" enctype="multipart/form-data"> @method('PUT') @endif
